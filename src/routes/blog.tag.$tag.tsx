...
const qo = (tag: string) =>
  queryOptions({
    queryKey: ["blog", "tag", tag],
    queryFn: () => listPublishedPosts(60, { tag }),
  });

export const Route = createFileRoute("/blog/tag/$tag")({
  loader: ({ context, params }) => context.queryClient.ensureQueryData(qo(params.tag)),
  head: ({ params }) => {
    const title = `#${params.tag} — Foxwood Blog`;
    const desc = `Posts tagged ${params.tag} on Foxwood Properties.`;
    return {
      meta: [
        { title },
  ...
  errorComponent: () => <div className="container-page py-24 text-center"><h1 className="text-2xl font-bold">Couldn't load tag</h1></div>,
  notFoundComponent: () => <div className="container-page py-24 text-center">Tag not found.</div>,
  component: TagPage,
});

function TagPage() {
  const { tag } = Route.useParams();
  const { data: posts } = useSuspenseQuery(qo(tag));
  return (
    <>
      <PageHero image={heroBlog} eyebrow="Tag" title={`#${tag}`} subtitle={`Articles tagged ${tag}.`} />
      <section className="container-page py-14">
        <div className="mb-6 text-sm">
          <Link to="/blog" className="text-primary hover:underline">← All posts</Link>
        </div>
        {posts.length === 0 ? (
          <div className="py-16 text-center text-muted-foreground inline-flex items-center gap-2"><TagIcon className="h-4 w-4" /> No posts for this tag yet.</div>
        ) : (
