...
import { GitCompare, X, Bed, Bath, Maximize, MapPin, Check, Minus } from "lucide-react";
      ...
      <section className="bg-primary-soft border-b border-border">
        <div className="container-page py-10 md:py-14 flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">Compare up to {max} properties side-by-side. {items.length}/{max} selected.</p>
          </div>
          {items.length > 0 && (
            <button onClick={clear} className="btn-ghost !py-2 !px-4 text-sm">Clear all</button>
          )}
        </div>
      </section>
                      ...
                      <div className="rounded-2xl border border-border bg-card overflow-hidden">
                        <div className="relative aspect-[4/3]">
                          ...
                          <button onClick={() => remove(p.id)}
                            className="absolute top-2 right-2 grid h-8 w-8 place-items-center rounded-full bg-background/95 shadow-soft hover:bg-destructive hover:text-destructive-foreground transition-colors"
                            aria-label="Remove">
                            <X className="h-4 w-4" />
                          </button>
